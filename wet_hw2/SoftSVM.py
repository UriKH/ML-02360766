...
class SoftSVM(BaseEstimator, ClassifierMixin):
    ...
    def loss(w, b: float, C: float, X, y):
        ...
        maximum = np.maximum(np.zeros(len(hinge_inputs)), 1 - hinge_inputs)
        loss = norm + np.multiply(C, np.sum(maximum))
        return loss
    ...
    def subgradient(w, b: float, C: float, X, y):
        ...
        applied_f = np.where(hinge_inputs < 1, -1, 0)

        g_w = np.multiply(2, w) + np.multiply(C, np.sum(np.multiply(applied_f.dot(X), y)))
        g_b = np.multiply(C, np.sum(np.multiply(applied_f, y)))
        return g_w, g_b
